...
class Dmart {
  ...
  static Future<(ActionResponse?, Error?)> query(QueryRequest query, {String scope = "managed"}) async {
  ...
  static Future<(ActionResponse?, Error?)> createSpace(ActionRequest action) async {
    _isTokenNull();
    try {
      final response = await _dio.post(
        '/managed/space',
